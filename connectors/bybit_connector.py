...
class EnhancedBybitConnector:
                        ...
                        if tickers:
                            ...
                            ticker = {
                                ...
                                "price_24h_pcnt": float(
                                    ticker_data.get("price24hPcnt", 0)
                                ),
    ...
    async def get_klines_batch(
        ...
        if tasks_to_fetch:
            logger.info(f"📊 Загружаем {len(tasks_to_fetch)} таймфреймов для {symbol}...")

            results = await asyncio.gather(*tasks_to_fetch, return_exceptions=True)

            # Сохраняем результаты в кеш
            for cache_key, tf, result in zip(cache_keys_to_fetch, timeframes_to_fetch, results):
        ...
        return {
            **self.batch_stats,
            'cache_hit_rate': (
                ...
                if (self.batch_stats['cache_hits'] + self.batch_stats['cache_misses']) > 0
                else 0.0
            )
        }
