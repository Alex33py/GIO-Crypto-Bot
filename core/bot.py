...
class GIOCryptoBot:
    ...
    async def initialize(self):
        ...
        try:
                    ...
                    if total_volume > 0:
                        ...
                        if hasattr(self, 'l2_imbalances'):
                            ...
                            if len(self.l2_imbalances["BTCUSDT"]) > 100:
                                self.l2_imbalances["BTCUSDT"] = self.l2_imbalances["BTCUSDT"][-100:]
            ...
            try:
                ...
                logger.info("ℹ️ filters_config не найден, используем дефолтные параметры")
                use_config = False
                CONFIRM_FILTER_CONFIG = {
                    "enabled": True,
                    "cvd_threshold": 0.5,
            ...
            if CONFIRM_FILTER_CONFIG.get("enabled", True):
                try:
                    ...
                    self.confirm_filter = ConfirmFilter(
                        ...
                        cvd_threshold=CONFIRM_FILTER_CONFIG.get("cvd_threshold", 0.5),
            ...
            if MULTI_TF_FILTER_CONFIG.get("enabled", True):
                try:
                    ...
                    self.multi_tf_filter = MultiTimeframeFilter(
                        ...
                        min_aligned_count=MULTI_TF_FILTER_CONFIG.get("min_aligned_count", 2),
                        higher_tf_weight=MULTI_TF_FILTER_CONFIG.get("higher_tf_weight", 2.0),
                    )
                    logger.info(
            ...
            # 8️⃣.5 Инициализация Telegram ROITracker для уведомлений
            ...
            self.telegram_roi_tracker = TelegramROITracker(
                ...
                bot_instance=self,  # ✅ ДОБАВЛЕНО: Передаём bot instance
            )
            logger.info("   ✅ Telegram ROITracker инициализирован с bot_instance")

            self.roi_tracker = (
                self.telegram_roi_tracker
            )  # ← ИСПОЛЬЗУЕМ TelegramROITracker!
            logger.info("   ✅ ROI Tracker установлен (TelegramROITracker)")
        ...
        try:
            ...
            if ba:
                ...
                logger.debug(
                    ...
                    f"Bid=${ba[0]:,.2f} Ask=${ba[1]:,.2f} Spread=${spread:.2f}"
                )
            ...
            if value > 10000:
                ...
                if hasattr(self, 'large_trades'):
                    ...
                    if len(self.large_trades[symbol_normalized]) > 200:
                        self.large_trades[symbol_normalized] = self.large_trades[symbol_normalized][-200:]
        ...
        try:
            ...
            if value > 10000:
                ...
                if hasattr(self, 'large_trades'):  # ← 12 ПРОБЕЛОВ!
                    ...
                    self.large_trades[symbol_normalized].append({  # ← 16 ПРОБЕЛОВ!
                        'price': trade['price'],
                        'quantity': trade['size'],
                        'side': trade['side'],
                        'timestamp': datetime.now()
    ...
    async def run(self):
        ...
        try:
            ...
            # Запуск ROI мониторинга
            if self.roi_tracker:
                try:
                    await self.roi_tracker.start_monitoring()
                    logger.info("✅ ROI мониторинг запущен")
                except Exception as e:
