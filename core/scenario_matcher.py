...
class UnifiedScenarioMatcher:
    ...
    def __init__(
        ...
        """
        Args:
            scenarios_path: Путь к JSON-файлу со сценариями
            deal_threshold: Порог для статуса DEAL (75%)
            risky_threshold: Порог для статуса RISKY_ENTRY (60%)
            observation_threshold: Порог для статуса OBSERVATION (35%)
        """
        # Путь к сценариям
        ...
        logger.info(
            ...
            f"({len(self.scenarios)} сценариев, "
    ...
    def _check_mtf_policy(
        ...
        """Проверка MTF условий (тренды на всех ТФ должны совпадать)"""
        try:
            # ВИПРАВЛЕНО: Отримуємо opinion з сценарію
            required_opinion = scenario.get(
                "opinion", "bullish"
            )  # "bullish" або "bearish"

            # Конвертуємо "bullish"/"bearish" у "uptrend"/"downtrend"
            if required_opinion == "bullish":
                required_trend = "uptrend"
            elif required_opinion == "bearish":
                required_trend = "downtrend"
            else:
                required_trend = required_opinion  # neutral або інше

            # Отримуємо тренди з mtf_trends або indicators
            if not isinstance(mtf_trends, dict):
                # Якщо mtf_trends не словарь - беремо з indicators
                trend_1h = indicators.get("trend_1h", "neutral")
                trend_4h = indicators.get("trend_4h", "neutral")
                trend_1d = indicators.get("trend_1d", "neutral")
            else:
                # Отримуємо з mtf_trends
                trend_1h_data = mtf_trends.get("1h", {})
                trend_4h_data = mtf_trends.get("4h", {})
                trend_1d_data = mtf_trends.get("1d", {})

                # Якщо значення - словарь, беремо "trend"
                if isinstance(trend_1h_data, dict):
                    trend_1h = trend_1h_data.get("trend", "neutral")
                else:
                    trend_1h = trend_1h_data if trend_1h_data else "neutral"

                if isinstance(trend_4h_data, dict):
                    trend_4h = trend_4h_data.get("trend", "neutral")
                else:
                    trend_4h = trend_4h_data if trend_4h_data else "neutral"

                if isinstance(trend_1d_data, dict):
                    trend_1d = trend_1d_data.get("trend", "neutral")
                else:
                    trend_1d = trend_1d_data if trend_1d_data else "neutral"

            # Fallback на indicators якщо пусто
            if not trend_1h or trend_1h == "neutral":
                trend_1h = indicators.get("trend_1h", "neutral")
            if not trend_4h or trend_4h == "neutral":
                trend_4h = indicators.get("trend_4h", "neutral")
            if not trend_1d or trend_1d == "neutral":
                trend_1d = indicators.get("trend_1d", "neutral")

            # ВИПРАВЛЕНО: Рахуємо збіги (ігноруємо регістр)
            aligned_count = 0
            ...
            # ВИПРАВЛЕНО: Підвищені score для часткових збігів
            if aligned_count == 3:
                return 1.0  # Всі 3 збігаються
            elif aligned_count == 2:
                return 0.7  # 2 збігаються
            elif aligned_count == 1:
                return 0.5  # ← ПІДВИЩЕНО з 0.4
            else:
                return 0.3  # ← ПІДВИЩЕНО з 0.2 (жоден не збігається)
