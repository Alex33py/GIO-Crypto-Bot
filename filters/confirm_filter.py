...
class ConfirmFilter:
    ...
    async def validate(
        self, symbol: str, direction: str, market_data: Optional[Dict] = None
    ) -> bool:
        """
    ...
    async def _check_cvd_simple(
        ...
        try:
            ...
            if direction == "LONG" and imbalance < 0:
                logger.debug(
                    f"   ⚠️ LONG сигнал, но CVD отрицательный ({imbalance:.1%})"
                )
                return False
            elif direction == "SHORT" and imbalance > 0:
                logger.debug(
                    f"   ⚠️ SHORT сигнал, но CVD положительный ({imbalance:.1%})"
                )
                return False

            logger.debug(
                f"   ✅ CVD проверка OK: {imbalance:.1%} (направление согласовано)"
            )
            return True
