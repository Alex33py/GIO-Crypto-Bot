...
class MultiTimeframeFilter:
    ...
    async def validate(
        ...
        **ОСНОВНОЙ МЕТОД** - Валидация сигнала через мультитаймфреймовый анализ
        ...
        try:
            ...
            logger.error(f"❌ Ошибка в MTF Filter.validate() для {symbol}: {e}", exc_info=True)
            return False, {}, f"❌ Ошибка валидации: {str(e)}"

    async def _get_mtf_data(self, symbol: str, timeframes: List[str]) -> Dict[str, Dict]:
        """
    ...
    async def _get_trend_from_mtf_analyzer(self, symbol: str, timeframe: str) -> Optional[Dict]:
        """
        Получает тренд из MTFAnalyzer бота

        Returns:
            Dict: {'trend': 'UP', 'strength': 0.85} или None
        """
        try:
            if not self.bot or not hasattr(self.bot, 'mtf_analyzer'):
                return None

            # Получаем klines из MTFAnalyzer
            klines = await self.bot.mtf_analyzer.get_klines(symbol, timeframe, limit=200)
    ...
    async def _calculate_trend_from_klines(self, symbol: str, timeframe: str) -> Optional[Dict]:
        """
    ...
    async def _get_klines_from_connector(self, symbol: str, timeframe: str) -> List[Dict]:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if data.get('retCode') == 0 and data.get('result', {}).get('list'):
                    ...
                    logger.info(f"✅ Получено {len(klines)} klines из Bybit REST для {symbol} {timeframe}")
                    return klines
                else:
                    logger.warning(f"⚠️ Bybit API error: {data.get('retMsg', 'Unknown error')}")
                    return []

        except Exception as e:
            logger.error(f"❌ Ошибка получения klines через REST API {symbol} {timeframe}: {e}")
            return []
